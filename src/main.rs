...
fn breadth_first_search(permutation_inicial: &[char]) -> Vec<char> {
    let n = permutation_inicial.len();
    let mut visitados = HashSet::new();

    visitados.insert(hash_permutation(permutation_inicial));
    let mut queue = VecDeque::new();
    queue.push_back((permutation_inicial.to_owned(), 0, 0));
    ...
    while let Some((permutation, index, level)) = queue.pop_front() {
        if is_pancake_sorted(&permutation) {
            ...
            println!("NIVEL: {}", level);
            println!("NÚMERO DE NODOS EN LA COLA: {}", queue.len());
        ...
        for i in 2..=n {
            ...
            if !visitados.contains(&sucesor_hash) {
                ...
                queue.push_back((sucesor, i - 1, level + 1));
            }
        }
    }
